...
IN_PROCESS_WORKER_NAME_PREFIX = "alert-workflow-chat-inline-"
...
def alert_workflow_chat_queue_name() -> str:
    settings = get_settings()
    base = settings.alert_workflow_chat_queue_name.strip() or "alert-workflow-chat"
    return f"{base}:{_queue_instance_fingerprint()}"


def alert_workflow_chat_job_id(run_id: str) -> str:
    return f"alert-workflow-chat-{_queue_instance_fingerprint()}-{run_id}"
...
def alert_workflow_chat_queue() -> Queue:
    return Queue(alert_workflow_chat_queue_name(), connection=redis_connection())


def alert_workflow_chat_stream_key(run_id: str) -> str:
    return f"alert-workflow-chat:run:{run_id}:events"


def alert_workflow_chat_cancel_key(run_id: str) -> str:
    return f"alert-workflow-chat:run:{run_id}:cancel"


def enqueue_alert_workflow_chat_run(run_id: str) -> str:
    settings = get_settings()
    queue = alert_workflow_chat_queue()
    job = queue.enqueue(
        "app.services.alert_workflow_chat.runner.run_alert_workflow_chat_job",
        run_id,
        job_id=alert_workflow_chat_job_id(run_id),
        job_timeout=settings.alert_workflow_chat_job_timeout_seconds,
        result_ttl=settings.alert_workflow_chat_result_ttl_seconds,
        failure_ttl=settings.alert_workflow_chat_result_ttl_seconds,
        description=f"alert workflow chat run {run_id}",
    )
    return str(job.id)


def request_alert_workflow_chat_cancel(run_id: str) -> None:
    settings = get_settings()
    redis_connection().setex(alert_workflow_chat_cancel_key(run_id), settings.alert_workflow_chat_result_ttl_seconds, "1")


def clear_alert_workflow_chat_cancel(run_id: str) -> None:
    redis_connection().delete(alert_workflow_chat_cancel_key(run_id))


def alert_workflow_chat_cancel_requested(run_id: str) -> bool:
    return bool(redis_connection().exists(alert_workflow_chat_cancel_key(run_id)))


def cancel_alert_workflow_chat_job(run_id: str) -> bool:
...
def ensure_alert_workflow_chat_job_queued(run_id: str) -> str:
    connection = redis_connection()
    queue = alert_workflow_chat_queue()
    job_id = alert_workflow_chat_job_id(run_id)
    ...
    try:
        ...
        return enqueue_alert_workflow_chat_run(run_id)
...
def alert_workflow_chat_job_status(run_id: str) -> str | None:
    try:
        job = Job.fetch(alert_workflow_chat_job_id(run_id), connection=redis_connection())
    except Exception:
        return None
    status = job.get_status(refresh=True)
...
def alert_workflow_chat_external_worker_available() -> bool:
    connection = redis_connection()
    queue = alert_workflow_chat_queue()
    return any(not bool(worker.get("is_in_process_fallback")) for worker in _queue_workers(queue, connection))


def alert_workflow_chat_queue_health() -> dict[str, object]:
    connection = redis_connection()
    queue = alert_workflow_chat_queue()
    ...
    return {
        ...
        "base_queue_name": get_settings().alert_workflow_chat_queue_name,
