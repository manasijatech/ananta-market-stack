...
3. Add the redirect URI in Upstox. For local development, use `http://localhost:3000/brokers`. This is your frontend `NEXT_PUBLIC_APP_URL` plus `/brokers`. In production, use the same route on your deployed frontend domain, for example `https://your-domain.com/brokers`.
...
4. Upstox redirects to your configured frontend URL, such as `http://localhost:3000/brokers?code=...`.
...
- Use `http://localhost:3000/brokers` for local development because this frontend has a real `/brokers` page and no dedicated `/callback/upstox` page yet.
