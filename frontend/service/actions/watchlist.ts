...
export async function replaceWatchlistSymbols(id: string, payload: WatchlistSymbolsReplaceInput): Promise<Watchlist> {
...
export async function removeSymbolFromWatchlist(
    ...
    const result = await request<Watchlist>(`/watchlists/${id}/symbols/${encodeURIComponent(symbol)}${query}`, {
        method: "DELETE"
    });
    revalidatePath("/watchlists");
    revalidatePath("/alerts-workspace/subscriptions");
