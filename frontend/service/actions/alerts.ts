...
export async function setAlertWorkflowStatus(id: string, status: "active" | "inactive"): Promise<AlertWorkflow> {
    const result = await request<AlertWorkflow>(
        `/alert-workflows/${id}/${status === "active" ? "enable" : "disable"}`,
        {
            method: "POST"
        }
    );
...
export async function testAlertWorkflow(
    id: string,
...
export async function sendWorkflowTestNotification(
    id: string,
    tick: Record<string, unknown>
...
export async function getAlertNotifications(
    params: { unread_only?: boolean; workflow_id?: string; limit?: number } = {}
...
export async function previewAlertUniverse(
    target_universe: Record<string, unknown>,
    limit = 50
): Promise<AlertUniversePreview> {
