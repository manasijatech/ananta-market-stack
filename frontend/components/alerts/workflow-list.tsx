...
export function WorkflowList({ emptyMessage, workflows }: { emptyMessage: string; workflows: AlertWorkflow[] }) {
        ...
        <section className="grid gap-4">
            ...
            {workflows.map((workflow) => (
                ...
                >
                    <div className="flex flex-wrap items-center justify-between gap-3">
                        <div>
                            ...
                            <div className="type-help mt-1 text-muted-foreground">
                                {workflowScope(workflow)} · {workflow.broker_code ?? "No broker"} ·{" "}
                                {workflow.workflow_dsl.targeting.mode.replaceAll("_", " ")}
                            </div>
                        </div>
                    ...
                    <div className="type-body mt-3 text-muted-foreground">
                        {workflow.description || "No description"}
                    </div>
                    {workflow.last_runtime_error ? (
                        <div className="type-meta mt-2 text-[var(--danger)]">{workflow.last_runtime_error}</div>
                    ) : null}
