...
export function MarketIntelligenceChrome({
 ...
 <span className="shrink-0 font-semibold uppercase tracking-[0.16em] text-primary">Watchlist</span>
 <Select
 ...
 <option key={group.id} value={group.id}>{group.name} ({group.symbols.length})</option>
 ))}
 </Select>
 </label>
 <span>
 {isLoadingFilter ? "Loading watchlist feed..." : `${filterLabel} / ${activeSymbols.length} symbols`}
 {activeSymbols.length > ALPHA_SYMBOL_LIMIT ? ` / first ${ALPHA_SYMBOL_LIMIT} used for history` : ""}
 </span>
 </div>
 ) : null}
