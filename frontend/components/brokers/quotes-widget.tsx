...
export function QuotesWidget({ account }: { account: BrokerAccount }) {
 ...
 setRows(result);
 if (!result.length) {
 setMessage(`No quote returned. Check the ${quoteIdentifierLabels[broker] ?? "broker identifier"} and try again.`);
 }
 } catch (error) {
 setRows([]);
 ...
 <p className="mt-4 border-t border-border py-4 text-sm text-muted-foreground">No quote data returned.</p>
 ) : null}
 </section>
 );
}
