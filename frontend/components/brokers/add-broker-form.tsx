    ...
    <div className="grid gap-6 lg:grid-cols-[320px_1fr]">
      <aside className="grid gap-3 self-start">
        {supportedBrokers.map((code) => (
          <button
            className={cn(
              "cursor-pointer rounded-lg border bg-card p-4 text-left transition hover:border-primary/40",
              broker === code && "border-primary bg-accent text-accent-foreground"
            )}
            key={code}
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Secrets are encrypted by the FastAPI backend before storage. Use the built-in docs for broker setup.
          </CardDescription>
        </CardHeader>
        ...
        <CardContent>
          ...
          {broker === "angel" ? (
            <>
              ...
              <Field error={fieldErrors.totp_secret} label="TOTP secret" name="totp_secret" optional />
            </>
          ) : null}
          ...
          {broker === "dhan" ? (
            <>
              ...
              <Field error={fieldErrors.app_secret} label="API secret" name="app_secret" type="password" />
          ...
          {broker === "kotak" ? (
            <>
              ...
              <Field error={fieldErrors.portal_access_token} label="Portal access token" name="portal_access_token" type="password" />
              <Field error={fieldErrors.mobile_number} label="Mobile number" name="mobile_number" optional />
          ...
          {broker === "groww" ? (
            <>
              <div className="flex flex-wrap gap-2">
                {(["approval", "totp", "token"] as GrowwMode[]).map((mode) => (
                  <Button
                    ...
                    type="button"
                  >
                    {mode === "approval" ? "API approval" : mode === "totp" ? "TOTP" : "Access token"}
                  </Button>
                ))}
              </div>
              {growwMode === "approval" ? (
              ...
              {growwMode === "token" ? (
                <Field error={fieldErrors.access_token} label="Access token" name="access_token" type="password" />
              ) : null}
            </>
          ) : null}
