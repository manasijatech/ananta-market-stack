...
export function BrokerDataTest({
 ...
 <StatusBadge className={account.last_verified_at ? "border-[var(--success)] bg-[var(--success-subtle)] text-[var(--success)]" : "border-[var(--accent)] bg-[var(--accent-subtle)] text-[var(--accent-dim)] dark:text-[var(--accent)]"}>
 ...
 <StatusBadge
 className={value.supported ? "border-[var(--success)] bg-[var(--success-subtle)] text-[var(--success)]" : "border-border bg-card text-muted-foreground"}
 key={key}
 >
 {key}
 </StatusBadge>
 ...
 <pre className="overflow-x-auto border border-border bg-muted/30 p-3 text-xs" key={`${index}-${JSON.stringify(message).slice(0, 24)}`}>
 {pretty(message)}
 </pre>
 ))}
 {!wsMessages.length ? (
