...
export function BrokerLogo({
    ...
    return (
        <span className={cn("flex h-12 w-12 shrink-0 items-center justify-center", className)} aria-hidden="true">
            <img
                alt=""
...
function sessionBadgeVariant(status?: string | null): NonNullable<BadgeProps["variant"]> {
    if (status === "active" || status === "automation_ready") return "success";
    if (!status || status === "pending" || status === "action_required") return "warning";
    return "secondary";
}
...
export function BrokerAccountsEmpty({ canAddBroker }: { canAddBroker: boolean }) {
    return (
        <CardFrame>
            <CardFrameHeader>
                <CardFrameTitle className={typography.h4}>Connected accounts</CardFrameTitle>
...
export function BrokerCard({ account }: { account: BrokerAccount }) {
    ...
    return (
        <CardFrame className="group w-full transition-colors duration-100 ease-out hover:border-primary/60">
            <CardFrameHeader>
                <CardFrameTitle className="truncate text-lg font-bold">{account.label}</CardFrameTitle>
                    ...
                    <div className="flex items-start gap-3">
                        ...
                        <div className="flex min-w-0 flex-1 flex-wrap gap-2">
                            ...
                            {account.automation_enabled ? (
                                <Badge variant="info">{account.automation_mode ?? "automation"}</Badge>
                            ) : null}
                        </div>
                    </div>
...
export function PrimaryLink({ href, children }: { href: string; children: React.ReactNode }) {
    return (
        <Button asChild className="min-h-11 w-full font-semibold min-[520px]:w-auto">
