...
): { render: DialogPrimitive.Trigger.Props["render"]; children: React.ReactNode } {
  if (!asChild || render) {
...
function resolveCloseAsChild(
  asChild: boolean | undefined,
  render: DialogPrimitive.Close.Props["render"],
  children: React.ReactNode,
): { render: DialogPrimitive.Close.Props["render"]; children: React.ReactNode } {
  if (!asChild || render) {
    return { render, children };
  }
...
export function DialogTrigger({
  ...
  const { render: resolvedRender, children: resolvedChildren } =
    resolveTriggerAsChild(asChild, render, children);
...
export function DialogClose({
  ...
  const { render: resolvedRender, children: resolvedChildren } =
    resolveCloseAsChild(asChild, render, children);
...
export function DialogBackdrop({
  ...
  return (
    <DialogPrimitive.Backdrop
      className={cn(
        "fixed inset-0 z-[80] bg-black/32 backdrop-blur-sm transition-all duration-200 data-ending-style:opacity-0 data-starting-style:opacity-0",
        className,
      )}
