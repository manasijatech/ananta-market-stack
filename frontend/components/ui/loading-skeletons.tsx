...
export function CardGridSkeleton({ count = 4, columns = "min-[960px]:grid-cols-2" }: { count?: number; columns?: string }) {
  return (
  ...
  return (
    <div className="grid gap-6">
      ...
      <section className="grid gap-4 border border-border p-5">
        ...
        {Array.from({ length: 3 }).map((_, index) => (
          <div className="grid gap-3 border border-border p-4 min-[900px]:grid-cols-[1fr_1fr_1fr_auto]" key={index}>
            <Skeleton className="h-10" />
            <Skeleton className="h-10" />
            <Skeleton className="h-10" />
            <Skeleton className="h-10 w-24" />
  ...
  return (
    <Shell>
      <PageHeader
        eyebrow="Workspace"
        title="Loading"
        description="Preparing your workspace view."
      />
