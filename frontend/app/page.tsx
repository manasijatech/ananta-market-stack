...
import { Activity, ArrowRight, CheckCircle2, Check, Copy, Lock, Route, Server, Zap } from "lucide-react";
...
const landingSymbols = Array.from(
    new Set([
...
function WorkflowFlowNode({ data }: NodeProps<Node<WorkflowNodeData>>) {
    return (
        <div className={`workflow-flow-node workflow-flow-node--${data.tone ?? "accent"}`}>
            {data.hasTarget ? (
                <Handle
                    className="workflow-flow-handle workflow-flow-handle--target"
            ...
            {data.hasSource ? (
                <Handle
                    className="workflow-flow-handle workflow-flow-handle--source"
                    ...
                    <span className="market-ticker__item" key={`${item.label}-${index}`}>
                        ...
                        {typeof item.changePercent === "number" ? (
                            <span
                                ...
                                style={{
                                    color: item.changePercent >= 0 ? "var(--landing-positive)" : "var(--landing-danger)"
                                }}
                            >
            ...
            <div className="liquidity-book">
                {orderRows.map((row, index) => (
                    <div className={`liquidity-row liquidity-row--${row.tone}`} key={row.price}>
                        ...
                        <span
                            className="liquidity-depth liquidity-depth--bid"
                            style={{ width: `${row.bid}%`, animationDelay: `${index * 130}ms` }}
                        />
                        <span
                            className="liquidity-depth liquidity-depth--ask"
                            style={{ width: `${row.ask}%`, animationDelay: `${index * 160}ms` }}
                        />
                            ...
                            >
                                ...
                                <div className="min-w-0">
                                    ...
                                    <div className="landing-overline">
                                        {broker.state} · {broker.latency}
                                    </div>
                                </div>
                                <IconCircleCheck
                                    className="landing-status-check ml-auto size-5"
                                    stroke={1.9}
            ...
            <div className="workflow-machine-layout mx-auto max-w-7xl px-5">
                <div className="workflow-machine-heading">
                    ...
                    <p className="landing-copy mt-5 max-w-md text-sm leading-6">
                        The product surface is not a notification widget. It is the execution machine behind the alert:
                        feed, stream, rule, optional model context, and channel acknowledgement.
                    </p>
                </div>
        ...
        <section className="landing-section">
            <div className="mx-auto grid max-w-7xl gap-12 min-[1040px]:grid-cols-[0.85fr_1.15fr]">
                <div>
                    ...
                    <p className="landing-copy mt-5 max-w-2xl text-sm leading-6 md:text-base">
                        Watchlists connect to news, announcements, earnings, concalls, broker instruments, and alert
                        workflows. The product surface is the market moving through your own filters.
                    </p>
                    <div className="landing-list mt-8 grid gap-3 font-mono text-xs">
                        <div className="flex items-center gap-3">
                            <CheckCircle2 className="size-4" /> Preset and manual watchlists
                        </div>
                        <div className="flex items-center gap-3">
                            <CheckCircle2 className="size-4" /> Live Alpha websocket feeds
                        </div>
                        <div className="flex items-center gap-3">
                            <CheckCircle2 className="size-4" /> Symbol metadata and instrument search
                        </div>
                    </div>
                </div>
                <div className="market-heatmap">
                    {heatmapTiles.map((tile, index) => (
                        <div
                            className={`market-tile market-tile--${tile.tone} ${tile.span}`}
                            key={tile.symbol}
                    ...
                    <div className="landing-hero-copy-zone max-w-3xl">
                        ...
                        <p className="landing-hero-copy max-w-2xl leading-7">
                            <span className="hero-copy-scrim">
                                Connect Indian broker accounts, stream market data, build alert workflows, and add LLM
                                analysis inside a private open-source workspace you control.
                            </span>
                        </p>
                        ...
                        <div className="landing-proof landing-hero-proof grid max-w-2xl gap-3 font-mono text-xs sm:grid-cols-3">
                            <span className="flex items-center gap-2">
                                <Lock className="size-4" /> encrypted secrets
                            </span>
                            <span className="flex items-center gap-2">
                                <Activity className="size-4" /> live alert workers
                            </span>
                            <span className="flex items-center gap-2">
                                <Server className="size-4" /> Docker deployable
                            </span>
                        </div>
                    </div>
                </div>
                ...
                <div className="self-host-shell mx-auto grid max-w-7xl gap-10 min-[980px]:grid-cols-[0.95fr_1.05fr] min-[980px]:items-center">
                    <div className="self-host-copy">
                        ...
                        <div className="self-host-proof mt-7">
                            ...
                            <span>
                                <Image
                                    src="/brand/tech/sqlitebrowser.png"
                                    alt=""
                                    width={18}
                                    height={18}
                                    className="object-contain"
