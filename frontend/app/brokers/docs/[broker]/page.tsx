    ...
    <Shell>
      <article className="mx-auto max-w-4xl">
        ...
        <ReactMarkdown
          ...
          components={{
            h1: () => null,
            h2: ({ children }) => <h2 className="mb-3 mt-10 text-xl font-bold tracking-tight">{children}</h2>,
            ...
            ul: ({ children }) => <ul className="mb-6 ml-5 list-disc space-y-2 text-sm text-muted-foreground">{children}</ul>,
            ol: ({ children }) => <ol className="mb-6 ml-5 list-decimal space-y-2 text-sm text-muted-foreground">{children}</ol>,
            li: ({ children }) => <li className="pl-1 leading-7">{children}</li>,
            img: ({ alt, src }) => (
              <img
                alt={alt ?? ""}
                className="mb-8 mt-4 w-full rounded-lg border bg-white"
