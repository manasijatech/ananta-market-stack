...
Phase 4 activates the reserved studio types (`alert-rule-draft` as a draft summary when `data.tool` is `alert_get_studio`, plus `workflow-graph`, `workflow-simulation`, `approval-card`). Adaptive chat can create a draft (`alert_create_draft` / `POST /adaptive-workspace/alert-studio/draft`) and confirm deploy (`alert_deploy_snapshot` with `confirm=true`). Studio payloads come from `GET/POST /adaptive-workspace/alert-studio*` and reuse `alert_workflow_chat_snapshots`. `adaptive_workspace_snapshots` stays layout-only. Full rule authoring stays on `/alerts-workspace`; that chat is unchanged.
