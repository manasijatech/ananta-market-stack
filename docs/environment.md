...
| `ENABLE_ADAPTIVE_WORKSPACE` | `false` | Runtime preview flag. When `false` (the default), Adaptive Workspace HTTP routes return `404`, the frontend hides `/adaptive-workspace`, and broker-chat runs never attach compose/intel/alert-studio tools. Enable only on local/preview instances with `ENABLE_ADAPTIVE_WORKSPACE=true`. Published Docker images stay off until a dedicated release. |
